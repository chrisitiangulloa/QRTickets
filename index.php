...
require('func.php');
/*
    $q = "select qrcode, instances from dbtickets where qrcode = '.$_GET['t'].'";
    	$r = $_SESSION['con']->query($q);

			while ($row = $r->fetch_assoc()) {
                $instances = $row['instances'];
            }

    if ($instances > 1) echo "ERROR: Boleto ha sido usado ya para este evento!";
    else {
        $q2 = "update table dbtickets set instances = " . $instances + 1 .  " where qrcode = '.$_GET['t'].'";
       	$r2 = $_SESSION['con']->query($q2);
        echo "Ticket valido. Exitosamente ingresado";
    }
*/
